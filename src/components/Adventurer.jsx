...
Command: npx gltfjsx@6.2.16 public/models/Adventurer.glb 
*/
...
export function Adventurer(props) {
  const { animation, nodes, materials, allAnimations } = props;
  const group = useRef()
  const { headFollow, cursrFollow, wireframe } = useControls({
    headFollow: false,
    cursrFollow: false,
    wireframe: false
  })
  ...
  useEffect(() => {
    actions[`CharacterArmature|${animation}`].reset().fadeIn(0.5).play();
    return () => {
      actions[`CharacterArmature|${animation}`].fadeOut(0.5);
    }
  }, [animation]);
